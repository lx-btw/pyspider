...
window.Debugger = (function() {
  ...
  return {
    ...
    not_saved: false,
    init_settings_editor: function($el) {
        var _this = this;
        this.python_editor_elem = $el;

        // Создаем элемент textarea вместо CodeMirror
        ...
        // Вставляем textarea в элемент $el
        $el[0].appendChild(textarea);
        ...
        textarea.addEventListener('input', function() { // Используем 'input' вместо 'change' для textarea
            _this.not_saved = true;
        });
    ...
    toggle_blank_mode: function(button) {
      if (!this.blank_mode) {
        if (this.not_saved) {
          if (!confirm("You have not saved changes. Ignore changes and switch to Blank mode.")) {
            return;
          }
          this.not_saved = false;
        }
        this.python_editor_elem.hide();  // Скрываем редактор кода
        $('#blank-sheet').show();  // Отображаем пустой лист
        $(button).addClass('active');
        this.blank_mode = !this.blank_mode;
      } else {
        var _this = this;
        console.log(location.pathname);
        $.ajax({
          ...
          url: location.pathname + '/get',
          success: function (data) {
            $('#blank-sheet').hide();  // Скрываем пустой лист
            _this.python_editor_elem.show();  // Отображаем редактор кода
            _this.python_editor.setValue(data.script);  // Восстанавливаем содержимое редактора
            _this.not_saved = false;
            $(button).removeClass('active');
            _this.blank_mode = !_this.blank_mode;
          },
          error: function() {
            alert('Loading script from database error. Script may be out-of-date.');
            $('#blank-sheet').hide();  // Скрываем пустой лист
            _this.python_editor_elem.show();  // Отображаем редактор кода
            $(button).removeClass('active');
            _this.blank_mode = !_this.blank_mode;
