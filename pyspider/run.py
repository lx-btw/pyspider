...
def read_config_from_server(ctx, param, value):
    if not value:
         return {}
     
    import requests
    response = requests.post("http://" + value + "/spyders?spdr=start") # 192.168.31.243:8000
    print(value)
...
@click.option('-cs', '--config_server', callback=read_config_from_server, type=click.STRING, # type: ignore
